...
def inference(images, keep_prob, batch_size):
    ...
    with tf.variable_scope('conv1') as scope:
        ...
        conv = tf.nn.conv2d(images, kernel, [1, 2, 2, 1], padding='SAME') #use_cudnn_on_gpu=True is default
    ...
    with tf.variable_scope('conv4') as scope:
        kernel = _variable_with_weight_decay('weights', shape=[5, 5, 128, 128],
                                             stddev=1e-4, wd=0.0)
    ...
    #norm3 = tf.nn.lrn(conv3, 4, bias=1.0, alpha=0.001 / 9.0, beta=0.75,
                     # name='norm3')
    # pool3
    '''pool4 = tf.nn.max_pool(conv6, ksize=[1, 3, 3, 1],
...
def loss(logits, labels):
    ...
    # decay terms (L2 loss).
    return tf.add_n(tf.get_collection('losses'), name='total_loss') #, tf.add_n(tf.get_collection('accuracy'), name='training_accuracy')
